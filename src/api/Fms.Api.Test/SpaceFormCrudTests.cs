...
public class SpaceFormCrudTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Admin_CreateSpace_SetsAuditColumns()
    {
        using var admin = AdminClient();
        ...
        // stamped by the Repository audit interceptor via HttpContext → ICurrentUserProvider.
        Assert.Equal(AdminUserIds, rows.Rows[0][0]!.ToString());
        Assert.NotEqual(DBNull.Value, rows.Rows[0][1]);
        Assert.NotEqual(DBNull.Value, rows.Rows[0][2]);
    }
