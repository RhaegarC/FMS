...
public class UserProvisionerTests
{
    ...
    public async Task ProvisionAsync_NewUser_ReturnsRoleUser()
    {
        ...
        var provisioner = new UserProvisioner(new UserRepository(db), Config());
    ...
    public async Task ProvisionAsync_UserInAdminUserIds_ReturnsRoleAdmin()
    {
        ...
        var provisioner = new UserProvisioner(new UserRepository(db), Config("oid-1,some-other"));
    ...
    public async Task ProvisionAsync_ExistingUserAddedToAdminList_PromotesToAdmin()
    {
        ...
        await new UserProvisioner(new UserRepository(db), Config()).ProvisionAsync(
            Principal("oid-1", "alice@example.com", "Alice"));

        var user = await new UserProvisioner(new UserRepository(db), Config("oid-1")).ProvisionAsync(
            Principal("oid-1", "alice@example.com", "Alice"));
    ...
    public async Task ProvisionAsync_RemovedFromAdminList_DoesNotDemote()
    {
        ...
        await new UserProvisioner(new UserRepository(db), Config("oid-1")).ProvisionAsync(
            Principal("oid-1", "alice@example.com", "Alice"));

        var user = await new UserProvisioner(new UserRepository(db), Config()).ProvisionAsync(
            Principal("oid-1", "alice@example.com", "Alice"));
