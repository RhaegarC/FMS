...
/// <summary>
/// RED for feature 02 (db-schema): the Repository layer must create the FMS schema
/// via EF Core migrations — tables, jsonb columns, and foreign keys. Runs against a
/// real PostgreSQL (Testcontainers) because migrations, FK constraints, and jsonb
/// types cannot be validated by the InMemory provider. Drives <see cref="FmsDbContext"/>
/// directly (this is the Repository layer's test project), no web host.
/// </summary>
public class DbSchemaTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Migrations_Apply_AndCreateAllTables()
    {
        await MigrateAsync();
